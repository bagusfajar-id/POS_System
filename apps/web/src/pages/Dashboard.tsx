...
interface RecentTransaction { id: string; invoiceNo: string; total: number; createdAt: string; user?: { name: string }; branch?: { name: string } }
interface Summary {
  todayRevenue: number
...
export default function Dashboard() {
  ...
  const chartData = monthly?.dailySummary
    ? Object.entries(monthly.dailySummary).map(([date, val]) => ({
        date: date.slice(5),
        revenue: val.revenue,
        trx: val.transactions
      }))
    : []
  ...
  return (
    <div className="fade-in">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 24 }}>
        <StatCard label="Pendapatan Hari Ini" value={formatRupiah(summary?.todayRevenue || 0)} icon="💰" color="var(--accent)" />
        <StatCard label="Transaksi Hari Ini" value={String(summary?.todayTransactions || 0)} icon="🧾" color="var(--accent-3)" sub="transaksi selesai" />
        <StatCard label="Total Produk" value={String(summary?.totalProducts || 0)} icon="📦" color="#43e97b" />
        <StatCard label="Stok Menipis" value={String(summary?.lowStockProducts || 0)} icon="⚠️" color="var(--accent-2)" sub="produk ≤ 5 unit" />
      </div>
        ...
        <div className="glass" style={{ padding: 24 }}>
          <div style={{ marginBottom: 20 }}>
            ...
            <p style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 2 }}>
              Total: {formatRupiah(monthly?.totalRevenue || 0)}
            </p>
          </div>
